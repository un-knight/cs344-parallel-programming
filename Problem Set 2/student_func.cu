                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  int tmp = filterWidth / 2;
  float result = 0.0f;
  for(int r = -tmp; r <= tmp; ++r){
    for(int c = -tmp; c <= tmp; ++c){
      int filter_pos = (r + tmp) * filterWidth + c + tmp;

      int img_r = 0;
      int img_c = 0;
      if(thread_2D_pos.y + r < 0)
      {
        img_r = 0;
      }
      else if(thread_2D_pos.y + r >= numRows)
      {
        img_r = numRows - 1;
      }
      else
      {
        img_r = thread_2D_pos.y + r;
      }

      if(thread_2D_pos.x + c < 0)
      {
        img_c = 0;
      }
      else if(thread_2D_pos.x + c >= numCols)
      {
        img_c = numCols - 1;
      }
      else
      {
        img_c = thread_2D_pos.x + c;
      }

      int img_pos = img_r * numCols + img_c;

      result += filter[filter_pos] * inputChannel[img_pos];
    }
  }
